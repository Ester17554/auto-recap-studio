...
const KEY="AUTO_RECAP_STUDIO_PROJECTS_V4";
...
function newProject(){state.current={id:crypto.randomUUID(),name:"",created:Date.now(),duration:0,script:"",audioDuration:null,rows:[],segments:[],status:"Novo",engine:"V4-mobile-lite"};runtime.movie=null;runtime.audio=null;$('projectName').value="";$('projectTitle').textContent="Novo projeto";$('scriptInput').value="";$('movieFile').value="";$('audioFile').value="";$('movieInfo').textContent="Nenhum filme selecionado.";$('audioInfo').textContent="Nenhuma narração selecionada.";$('analysisBox').classList.add('hidden');setProgress(0);go('project')}
function openProject(p){state.current=JSON.parse(JSON.stringify(p));runtime.movie=null;runtime.audio=null;$('projectName').value=p.name||'Projeto';$('projectTitle').textContent=p.name||'Projeto';$('scriptInput').value=p.script||'';$('movieFile').value='';$('audioFile').value='';$('movieInfo').textContent=p.duration?`Filme analisado · ${tc(p.duration)}`:'Selecione o filme novamente para gerar novos frames.';$('audioInfo').textContent=p.audioDuration?`Narração · ${tc(p.audioDuration)}`:'Nenhuma narração salva.';if(p.rows?.length){renderMap();go('map')}else go('project')}
...
function audioDuration(file){if(!file)return Promise.resolve(null);return new Promise(res=>{const a=document.createElement('audio');a.preload='metadata';const u=URL.createObjectURL(file);a.onloadedmetadata=()=>{const d=a.duration;URL.revokeObjectURL(u);res(isFinite(d)?d:null)};a.onerror=()=>{URL.revokeObjectURL(u);res(null)};a.src=u})}
...
async function analyze(){const movie=$('movieFile').files[0],audio=$('audioFile').files[0],script=$('scriptInput').value;if(!movie||!script.trim()){toast('Selecione o filme e cole o roteiro.');return}const step=Number($('sampleStep').value),topk=Number($('candidateCount').value),sens=Number($('cutSensitivity').value),maxN=Number($('maxChunks').value);$('analyzeBtn').disabled=true;try{setProgress(1);setAnalysis('Lendo arquivos','Abrindo apenas os metadados...');const dur=await duration(movie);const ad=await audioDuration(audio);runtime.movie=movie;runtime.audio=audio;state.current.duration=dur;state.current.audioDuration=ad;state.current.script=script;state.current.name=($('projectName').value||'Projeto').trim();$('movieInfo').textContent=`${movie.name} · ${tc(dur)}`;$('audioInfo').textContent=audio?`${audio.name} · ${(audio.size/1048576).toFixed(1)} MB${ad?` · ${tc(ad)}`:' · duração não lida pelo Safari'}`:'Nenhuma narração.';
...
const chunks=splitScript(script,maxN);const rows=[];for(let ci=0;ci<chunks.length;ci++){const text=chunks[ci];const nt=ad?Math.min(dur,(ci/chunks.length)*ad):((ci+.5)/chunks.length)*dur;const target=ad?((ci+.5)/chunks.length)*dur:((ci+.5)/chunks.length)*dur;const center=seg.reduce((best,s,i)=>Math.abs(s.mid-target)<Math.abs(seg[best].mid-target)?i:best,0);const candidates=[];for(let d=-10;d<=10;d++){const idx=center+d;if(seg[idx]){const s=seg[idx],near=Math.exp(-Math.abs(s.mid-target)/Math.max(20,dur*.18));candidates.push({s,score:.55*near+keywordScore(text,s)})}}candidates.sort((a,b)=>b.score-a.score);const chosen=[];for(const x of candidates){if(chosen.some(y=>Math.abs(y.s.mid-x.s.mid)<step*.8))continue;chosen.push(x);if(chosen.length>=topk)break}chosen.forEach((x,rank)=>{const s=x.s;const score=Math.min(.95,.35+x.score*.6);rows.push({trecho:ci+1,candidato:rank+1,roteiro:text,narracao_inicio:ad?(ci/chunks.length)*ad:null,narracao_fim:ad?((ci+1)/chunks.length)*ad:null,filme_inicio:s.start,filme_fim:s.end,frame:s.mid,score:Number(score.toFixed(3)),confianca:'GUIA',cena_id:`CENA_${String(s.id).padStart(4,'0')}`,imagem:s.url,aprovada:false,metodo:'V4-lite · proximidade temporal + detecção de mudança'});});setProgress(60+(ci+1)/chunks.length*38);setAnalysis('Montando mapa',`Trecho ${ci+1} de ${chunks.length}`);await new Promise(r=>setTimeout(r,0))}
state.current.rows=rows;state.current.status='Mapa pronto';state.current.engine='V3-lite';saveCurrent();renderMap();setProgress(100);go('map');toast('Mapa V3 criado');URL.revokeObjectURL(v.src)}catch(e){console.error(e);setAnalysis('Não foi possível concluir',e.message||'Erro');toast('A análise parou. Veja a mensagem acima.')}finally{$('analyzeBtn').disabled=false}}
function renderMap(){const p=state.current;if(!p)return;$('mapTitle').textContent=p.name||'Projeto';const rows=p.rows||[];const groups=new Map();rows.forEach(r=>{if(!groups.has(r.trecho))groups.set(r.trecho,[]);groups.get(r.trecho).push(r)});$('statChunks').textContent=groups.size;$('statScenes').textContent=rows.length;$('statApproved').textContent=rows.filter(r=>r.aprovada).length;const box=$('mapList');box.innerHTML='';for(const [n,arr] of groups){const sec=document.createElement('div');sec.className='segment';sec.innerHTML=`<div class="segmentTop"><div class="segmentTitle">TRECHO ${pad(n)}</div><div class="segmentTitle">${arr.length} candidatos</div></div><div class="segmentText">${esc(arr[0].roteiro)}</div><div class="candidateGrid"></div>`;const grid=sec.querySelector('.candidateGrid');arr.forEach((r,i)=>{const c=document.createElement('div');c.className='candidate'+(r.aprovada?' approved':'');c.innerHTML=`<img src="${r.imagem}" loading="lazy"><div class="candidateBody"><div class="rank">CANDIDATO ${i+1} · ${esc(r.cena_id)}</div><div class="confidence">${r.confianca} · ${r.score}${r.aprovada?'<span class="approvedMark">✓ APROVADA</span>':''}</div><div class="time">Filme: <b>${tc(r.filme_inicio)} → ${tc(r.filme_fim)}</b><br>Frame: ${tc(r.frame)}</div><button class="smallBtn">${r.aprovada?'Desaprovar':'✓ Usar esta cena'}</button></div>`;c.querySelector('button').onclick=()=>{r.aprovada=!r.aprovada;persistCurrent();renderMap()};grid.appendChild(c)});box.appendChild(sec)}}
