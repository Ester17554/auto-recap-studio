...
const KEY="AUTO_RECAP_STUDIO_PROJECTS_V6";
let state={projects:[],current:null};
let runtime={movie:null,audioFiles:[],libraryFiles:[]};
...
function newProject(){state.current={id:crypto.randomUUID(),name:"",created:Date.now(),duration:0,script:"",audioDuration:null,audioParts:[],rows:[],segments:[],status:"Novo",engine:"V6-mobile-lite"};runtime.movie=null;runtime.audioFiles=[];$('projectName').value="";$('projectTitle').textContent="Novo projeto";$('scriptInput').value="";$('movieFile').value="";$('moviePhotoFile').value="";$('audioFile').value="";$('mediaLibrary').value="";runtime.libraryFiles=[];$('movieInfo').textContent="Nenhum filme selecionado.";$('audioInfo').textContent="Nenhuma narração selecionada.";$('audioList').innerHTML="";$('libraryList').innerHTML="";$('libraryInfo').textContent="Você pode adicionar trailers, cortes e imagens. O filme completo continua sendo o arquivo principal.";$('moviePreview').classList.add('hidden');$('moviePreviewVideo').removeAttribute('src');$('analysisBox').classList.add('hidden');setProgress(0);go('project')}
function openProject(p){state.current=JSON.parse(JSON.stringify(p));runtime.movie=null;runtime.audioFiles=[];$('projectName').value=p.name||'Projeto';$('projectTitle').textContent=p.name||'Projeto';$('scriptInput').value=p.script||'';$('movieFile').value='';$('moviePhotoFile').value='';$('audioFile').value='';$('mediaLibrary').value='';runtime.libraryFiles=[];$('movieInfo').textContent=p.duration?`Filme analisado · ${tc(p.duration)}`:'Selecione o filme novamente para gerar novos frames.';$('audioInfo').textContent=p.audioDuration?`Narração virtual · ${tc(p.audioDuration)} · ${p.audioParts?.length||0} arquivo(s)`:'Nenhuma narração salva.';$('audioList').innerHTML='';if(p.rows?.length){renderMap();go('map')}else go('project')}
...
function inspectMovie(file){
  ...
  info.textContent=`✅ Arquivo recebido: ${file.name}`;
  diag.classList.remove('hidden');
  diag.innerHTML=`<b>✅ O iPhone entregou o filme ao V6.</b><br>${mb} MB · ${file.type||'tipo não informado'}<br><span>Testando apenas a duração; o filme inteiro não é enviado para a internet.</span>`;
  const pv=$('moviePreviewVideo');
  try{if(pv.dataset.url)URL.revokeObjectURL(pv.dataset.url);const u=URL.createObjectURL(file);pv.dataset.url=u;pv.src=u;$('moviePreview').classList.remove('hidden')}catch(e){}
  duration(file).then(d=>{
    state.current.duration=d;
    info.textContent=`✅ ${file.name} · ${mb} MB · duração ${tc(d)}`;
    diag.innerHTML=`<b>✅ Filme reconhecido.</b><br>${mb} MB · ${file.type||'tipo não informado'} · ${tc(d)}<br><span>Agora você pode criar o mapa.</span>`;
  }).catch(e=>{
    info.textContent=`⚠️ ${file.name} · ${mb} MB · arquivo selecionado`;
    diag.innerHTML=`<b>⚠️ Arquivo selecionado, mas o Safari não leu os metadados.</b><br>${mb} MB · ${file.type||'tipo não informado'}<br><span>${esc(e.message)}</span>`;
  });
}
function showLibrary(files){
  runtime.libraryFiles=Array.from(files||[]);
  const list=$('libraryList'); list.innerHTML='';
  if(!runtime.libraryFiles.length){$('libraryInfo').textContent='Nenhum item selecionado.';return}
  runtime.libraryFiles.forEach((f,i)=>{const row=document.createElement('div');row.className='audioRow';row.innerHTML=`<div class="audioRowMain"><b>${i+1}. ${esc(f.name)}</b><span>${(f.size/1048576).toFixed(1)} MB · ${esc(f.type||'mídia')}</span></div></div>`;list.appendChild(row)});
  $('libraryInfo').textContent=`${runtime.libraryFiles.length} item(ns) na biblioteca visual. Eles ficam disponíveis nesta sessão.`;
}
...
async function inspectAudioFiles(files,append=false){
  const incoming=Array.from(files||[]);
  if(append) runtime.audioFiles=[...runtime.audioFiles,...incoming]; else runtime.audioFiles=incoming;
  ...
  list.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.remove);runtime.audioFiles.splice(i,1);syncAudioInput();inspectAudioFiles(runtime.audioFiles,false)});
...
function addAudioPicker(){const el=$('audioFile');el.value='';el.click()}
...
function exportJson(){download(`${state.current.name||'projeto'}_mapa_v6.json`,JSON.stringify(state.current,null,2),'application/json')}
function exportCsv(){const h='trecho,candidato,cena_id,narracao_inicio,narracao_fim,narracao_arquivos,filme_inicio,filme_fim,frame,score,confianca,aprovada,metodo,roteiro\n';const r=state.current.rows.map(x=>[x.trecho,x.candidato,x.cena_id,x.narracao_inicio??'',x.narracao_fim??'',`"${(x.narracao_arquivos||[]).join(' + ').replace(/"/g,'""')}"`,x.filme_inicio,x.filme_fim,x.frame,x.score,x.confianca,x.aprovada?'SIM':'NAO',x.metodo,`"${String(x.roteiro).replace(/"/g,'""')}"`].join(','));download(`${state.current.name||'projeto'}_mapa_v6.csv`,h+r.join('\n'),'text/csv')}
function exportPlan(){const rows=state.current.rows.filter(r=>r.aprovada).sort((a,b)=>a.trecho-b.trecho);let out=`AUTO RECAP STUDIO V6 — PLANO CAPCUT\nProjeto: ${state.current.name}\n\n`;rows.forEach((r,i)=>{out+=`CENA ${String(i+1).padStart(3,'0')}\nRoteiro: ${r.roteiro}\nNarração: ${r.narracao_inicio!=null?tc(r.narracao_inicio):'--:--'} -> ${r.narracao_fim!=null?tc(r.narracao_fim):'--:--'}\nÁudio: ${(r.narracao_arquivos||[]).join(' + ')}\nFilme: ${tc(r.filme_inicio)} -> ${tc(r.filme_fim)}\nFrame: ${tc(r.frame)}\nMétodo: ${r.metodo}\n\n`});if(!rows.length)out+='Nenhuma cena aprovada ainda.\n';download(`${state.current.name||'projeto'}_plano_capcut_v6.txt`,out,'text/plain')}
function showExport(){go('export')}
function wire(){
  $('startBtn').onclick=newProject;
  $('newProject').onclick=newProject;
  $('saveProject').onclick=saveCurrent;
  $('analyzeBtn').onclick=analyze;
  const moviePicked=f=>{if(f){runtime.movie=f;inspectMovie(f)}else toast('Nenhum filme foi entregue pelo seletor. Tente o outro botão.')};
  $('movieFile').addEventListener('change',()=>moviePicked($('movieFile').files[0]));
  $('movieFile').addEventListener('input',()=>moviePicked($('movieFile').files[0]));
  $('movieFile').addEventListener('cancel',()=>toast('O seletor foi fechado sem entregar o filme. Tente "Fototeca" ou "Arquivos".'));
  $('moviePhotoFile').addEventListener('change',()=>moviePicked($('moviePhotoFile').files[0]));
  $('moviePhotoFile').addEventListener('input',()=>moviePicked($('moviePhotoFile').files[0]));
  $('moviePhotoFile').addEventListener('cancel',()=>toast('A Fototeca não entregou o vídeo. Tente "Arquivos".'));
  $('pickMovieFiles').onclick=()=>{$('movieFile').click()};
  $('pickMoviePhotos').onclick=()=>{$('moviePhotoFile').click()};
  $('addAudioBtn').onclick=addAudioPicker;
  $('audioFile').addEventListener('change',()=>{const files=Array.from($('audioFile').files||[]);if(files.length)inspectAudioFiles(files,true)});
  $('audioFile').addEventListener('cancel',()=>toast('Nenhum áudio foi adicionado.'));
  $('addLibraryBtn').onclick=()=>{$('mediaLibrary').click()};
  $('mediaLibrary').addEventListener('change',()=>showLibrary(Array.from($('mediaLibrary').files||[])));
  $('scriptInput').addEventListener('input',updateScriptInfo);
  $('approveAll').onclick=approveBest;$('clearApproved').onclick=clearApproved;$('exportBtn').onclick=showExport;$('exportTop').onclick=showExport;$('capcutBtn').onclick=()=>{showExport();toast('Aprove as cenas e baixe o plano CapCut')};$('downloadJson').onclick=exportJson;$('downloadCsv').onclick=exportCsv;$('downloadPlan').onclick=exportPlan;
  document.querySelectorAll('[data-back]').forEach(b=>b.onclick=()=>go(b.dataset.back));document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>go(b.dataset.nav));
}

loadState();wire();renderProjects();
// V6: disable old service workers/caches so the iPhone does not keep an older picker UI.
